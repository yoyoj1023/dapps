...
export const usePaginatedMessages = (
  ...
  const totalPages = totalMessages ? Math.ceil(Number(totalMessages) / config.pageSize) : 0;
  ...
  const currentPageMessages = latestMessages
    ? (latestMessages as any[]).slice(currentPage * config.pageSize, (currentPage + 1) * config.pageSize)
    : [];
