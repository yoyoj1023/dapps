...
const MessageSchema = z.object({
  text: z.string().min(1, "留言內容不能為空").max(5000, "留言內容不能超過 5000 字"),
  author: z.string().max(100, "作者名稱不能超過 100 字").optional(),
  title: z.string().max(200, "標題不能超過 200 字").optional(),
  tags: z.array(z.string().max(50)).max(10, "標籤數量不能超過 10 個").optional(),
});
...
async function uploadToPinata(data: any, apiKey: string, apiSecret: string): Promise<PinataResponse> {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const pinataResponse = await uploadToPinata(messageContent, apiKey, apiSecret);
