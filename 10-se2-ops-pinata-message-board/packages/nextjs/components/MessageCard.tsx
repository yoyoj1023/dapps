  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-4 transition-all hover:shadow-lg">
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          {content?.title && <h3 className="text-lg font-semibold text-gray-900 mb-2">{content.title}</h3>}
          <div className="flex items-center space-x-4 text-sm text-gray-500">
            <span className="flex items-center">👤 {content?.author || formatAddress(message.sender)}</span>
            <span className="flex items-center">🕒 {formatTimestamp(message.timestamp)}</span>
            <span className="text-xs bg-gray-100 px-2 py-1 rounded">#{message.messageId}</span>
          </div>
        </div>

        {/* CID 顯示 */}
        <div className="text-xs text-gray-400">
          <span className="font-mono bg-gray-50 px-2 py-1 rounded">{message.ipfsCid.substring(0, 10)}...</span>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-red-50 border border-red-200 rounded-md p-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={retry} className="text-red-600 hover:text-red-800 text-sm underline">
                重試
              </button>
            </div>
          </div>
      ...
      <div className="border-t pt-3 flex justify-between items-center text-xs text-gray-400">
        <div className="flex space-x-4">
          ...
          {content?.createdAt && <span>建立: {new Date(content.createdAt).toLocaleDateString("zh-TW")}</span>}
        </div>
